        ...
        <div className="mt-6 grid gap-8 lg:grid-cols-[.9fr_1.1fr]">
          <section className="rounded-3xl bg-white p-7">
            <div className="flex items-center justify-between">
              <div><p className="text-xs font-bold uppercase tracking-wider text-[#a05247]">Partner workspace</p><h1 className="mt-1 font-heading text-2xl font-bold">Product sample edit</h1></div>
            ...
            {state.rfqAssortment.length ? (
              ...
              <div className="py-16 text-center text-[#657068]"><ClipboardList className="mx-auto" size={34} /><p className="mt-3">No beauty products added yet.</p><Link to="/products" className="mt-3 inline-block font-semibold text-[#a05247]">Browse the collection</Link></div>
            )}
            <p className="mt-5 text-xs leading-5 text-[#657068]">Quantities are planning inputs only. Final formula, shade and packaging minimums are confirmed in the quotation.</p>
          </section>

          <form onSubmit={submit} className="rounded-3xl bg-white p-7">
            <h2 className="font-heading text-2xl font-bold">Beauty partnership brief</h2>
            <div className="mt-6 space-y-5">
              ...
              <label className="block"><span className="mb-2 block text-sm font-semibold">Channel, audience and price positioning *</span><textarea required rows="3" value={form.targetCustomerProfile} onChange={(event) => setForm({ ...form, targetCustomerProfile: event.target.value })} placeholder="e.g. DTC clean-beauty retailer, ages 25–45, $28–$65" className="w-full rounded-xl" /></label>
              <label className="block"><span className="mb-2 block text-sm font-semibold">Formula, shades and packaging requirements *</span><textarea required rows="4" value={form.specifications} onChange={(event) => setForm({ ...form, specifications: event.target.value })} placeholder="e.g. barrier skincare trio, vegan formulas, custom cartons" className="w-full rounded-xl" /></label>
              <label className="block"><span className="mb-2 block text-sm font-semibold">Estimated total opening units *</span><input required type="number" min="1" value={form.estimatedQuantity} onChange={(event) => setForm({ ...form, estimatedQuantity: event.target.value })} className="w-full rounded-xl" /></label>
              <label className="block"><span className="mb-2 block text-sm font-semibold">Target retail price, margin, delivery and other notes</span><textarea rows="4" value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} className="w-full rounded-xl" /></label>
              ...
              <button disabled={status.sending || !state.rfqAssortment.length} className="w-full rounded-full bg-[#2d201d] py-4 font-semibold text-white disabled:opacity-40">{status.sending ? 'Sending…' : 'Submit partnership inquiry'}</button>
            </div>
          </form>
        </div>
      </main>
