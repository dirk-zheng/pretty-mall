...
export default function HomePage() {
  ...
  return <div className="min-h-screen bg-[#fbf7f2] pt-24">
      ...
      <div className="relative mx-auto flex min-h-[720px] max-w-7xl items-center px-5 sm:px-8">
        <div className="max-w-xl py-20">
          ...
          <div className="mt-9 flex flex-wrap gap-3">
            <Link to="/products" className="inline-flex items-center gap-2 rounded-full bg-[#2d201d] px-7 py-4 font-semibold text-white transition hover:bg-[#a05247]">View product portfolio <ArrowRight size={18}/></Link>
    ...
    <section className="mx-auto max-w-7xl px-5 py-24 sm:px-8">
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.24em] text-[#a05247]">The Aurelia edit</p><h2 className="mt-3 font-heading text-4xl font-medium text-[#2d201d] sm:text-5xl">Small rituals. Visible radiance.</h2><p className="mt-4 max-w-2xl leading-7 text-[#74615b]">An edited portfolio of skincare, color and scent for retail, gifting and private-label programs.</p></div><Link to="/products" className="inline-flex items-center gap-2 font-semibold text-[#7d3f38]">View all products <ArrowRight size={17}/></Link></div>
      <div className="mt-11 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">{featured.map((product,index)=><Link key={product.id} to={`/products/${productSlug(product)}`} className="group"><div className="relative overflow-hidden rounded-[2rem] bg-[#eadfd8]"><img src={product.image} alt={product.name} className={`aspect-[4/5] w-full object-cover transition duration-700 group-hover:scale-105 ${index%3===1?'object-[58%_center]':index%3===2?'object-[78%_center]':'object-[25%_center]'}`}/><span className="absolute left-4 top-4 rounded-full bg-white/90 px-3 py-1.5 text-[11px] font-bold uppercase tracking-wider text-[#8d4940] backdrop-blur">{product.badge}</span></div><div className="mt-4 flex items-start justify-between gap-4"><div><h3 className="font-heading text-xl font-medium text-[#2d201d]">{product.name}</h3><p className="mt-1 text-sm text-[#7a6963]">{product.benefit}</p><p className="mt-1 text-xs text-[#8a7770]">Typical MOQ: {product.moq}</p></div><span className="shrink-0 text-xs font-semibold uppercase tracking-wider text-[#8d4940]">B2B</span></div></Link>)}</div>
    </section>

    <section className="bg-[#efe2da] py-24"><div className="mx-auto grid max-w-7xl items-center gap-12 px-5 sm:px-8 lg:grid-cols-2"><div className="overflow-hidden rounded-[2.5rem]"><img src="/beauty/skincare-ritual.png" alt="Aurelia skincare ritual" className="aspect-[4/5] w-full object-cover"/></div><div className="lg:pl-10"><Sparkles className="text-[#a05247]" size={30}/><p className="mt-6 text-xs font-bold uppercase tracking-[.24em] text-[#a05247]">Skin-first assortment</p><h2 className="mt-3 font-heading text-4xl font-medium leading-tight text-[#2d201d] sm:text-5xl">Build a ritual around changing skin needs.</h2><p className="mt-6 max-w-xl text-lg leading-8 text-[#6e5b55]">Combine essence, serum and cream into a coherent hydration story for retail shelves, discovery sets or custom programs.</p><Link to="/services/skin-ritual" className="mt-8 inline-flex items-center gap-2 border-b border-[#2d201d] pb-1 font-semibold">Explore the skin ritual <ArrowRight size={17}/></Link></div></div></section>
